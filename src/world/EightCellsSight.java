...
public class EightCellsSight implements Sight {
	...
	int processCell(int x, int y, int dx, int dy, World world, int i, int mask) {
		...
		int ny = y + dy;
		// torus
		if (nx >= world.height) nx -= world.height;
		if (nx < 0) nx += world.height;
		if (ny >= world.width) ny -= world.width;
		if (ny < 0) ny += world.width;
		
		return isFoodFunction.test(world.field[nx][ny]) ? (mask | (1 << i)) : mask; 
	}
}
