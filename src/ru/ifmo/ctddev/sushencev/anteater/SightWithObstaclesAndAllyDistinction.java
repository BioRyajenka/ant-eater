...
public class SightWithObstaclesAndAllyDistinction extends SimpleSight {
	...
	public SightWithObstaclesAndAllyDistinction(SerializablePredicate<Cell> isFoodFunction,
			int range, SerializablePredicate<Cell> isObstacleFunction) {
	...
	public InputSignal check(Cell[][] field, Position position, WorldGenerator wg) {
		...
		if (fp == null || isObstacleFunction.test(field[fp.second][fp.first])) {
			mask += super.getInputSignalsNumber();// ally = obstacle
		} else {
			if (field[fp.second][fp.first].hasIndividual()) {
				mask += 2 * super.getInputSignalsNumber();//enemy != obstacle
			}
		}
